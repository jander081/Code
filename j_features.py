...
def line_tot_prem(dframe_curr, q_num_curr, dframe_prev, q_num_prev):
    
    '''Quarterly agent line total written premiums'''
    
   
    # PIVOTs
    curr_agnt_q_pv = pd.pivot_table(dframe_curr, values='prem_', index='agnt_', 
                                   columns = ['quarter_', 'line_'], aggfunc='sum').fillna(0)
    
    prev_agnt_q_pv = pd.pivot_table(dframe_prev, values='prem_', index='agnt_', 
                                   columns = ['quarter_', 'line_'], aggfunc='sum').fillna(0)
    
    
    # AUTO CURRENT
    dict_agnt_curr_auto = pd.Series(curr_agnt_q_pv.loc[:, (q_num_curr, 'AUTO')], index=curr_agnt_q_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_auto_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_auto)
    # PREV      
    dict_agnt_prev_auto = pd.Series(prev_agnt_q_pv.loc[:, (q_num_prev, 'AUTO')], index=prev_agnt_q_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_auto_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_auto) 
       
    # CMP CURRENT 
    dict_agnt_curr_cmp = pd.Series(curr_agnt_q_pv[q_num_curr, 'COMM MULTI-PERIL'], index=curr_agnt_q_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_cmp_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_cmp)                     
    # PREV        
    dict_agnt_prev_cmp = pd.Series(prev_agnt_q_pv.loc[:, (q_num_prev, 'COMM MULTI-PERIL')], index=prev_agnt_q_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_cmp_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_cmp) 
    
    # WC CURRENT    
    dict_agnt_curr_wc = pd.Series(curr_agnt_q_pv[q_num_curr, 'WORKERS COMP'], index=curr_agnt_q_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_wc_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_wc)                     
    # PREV  
    dict_agnt_prev_wc = pd.Series(prev_agnt_q_pv[q_num_prev, 'WORKERS COMP'], index=prev_agnt_q_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_wc_tot_prem'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_wc) 
...
def line_num_acc(dframe_curr, q_num_curr, dframe_prev, q_num_prev):  
    
    '''Quarterly agent number of accounts by line'''
       
    # PIVOTs
    curr_agnt_q_num_pv = pd.pivot_table(dframe_curr, values='pol_num', index='agnt_', 
                                   columns = ['quarter_', 'line_'], aggfunc='count').fillna(0)
    
    prev_agnt_q_num_pv = pd.pivot_table(dframe_prev, values='pol_num', index='agnt_', 
                                   columns = ['quarter_', 'line_'], aggfunc='count').fillna(0)
      
    # AUTO CURRENT
    dict_agnt_curr_num_auto = pd.Series(curr_agnt_q_num_pv.loc[:, (q_num_curr, 'AUTO')], index=curr_agnt_q_num_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_auto_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_num_auto)
    # PREV      
    dict_agnt_prev_num_auto = pd.Series(prev_agnt_q_num_pv.loc[:, (q_num_prev, 'AUTO')], index=prev_agnt_q_num_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_auto_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_num_auto) 
       
    
    # CMP CURRENT
    dict_agnt_curr_num_cmp = pd.Series(curr_agnt_q_num_pv.loc[:, (q_num_curr, 'COMM MULTI-PERIL')], index=curr_agnt_q_num_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_cmp_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_num_cmp)
    # PREV      
    dict_agnt_prev_num_cmp = pd.Series(prev_agnt_q_num_pv.loc[:, (q_num_prev, 'COMM MULTI-PERIL')], index=prev_agnt_q_num_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_cmp_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_num_cmp) 
       
    # WC CURRENT
    dict_agnt_curr_num_wc = pd.Series(curr_agnt_q_num_pv.loc[:, (q_num_curr, 'WORKERS COMP')], index=curr_agnt_q_num_pv.index).to_dict()    
    dframe_curr.loc[:, 'curr_q_agnt_wc_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_curr_num_wc)
    # PREV      
    dict_agnt_prev_num_wc = pd.Series(prev_agnt_q_num_pv.loc[:, (q_num_prev, 'WORKERS COMP')], index=prev_agnt_q_num_pv.index).to_dict()
    dframe_curr.loc[:, 'prev_q_agnt_wc_num_acc'] = dframe_curr.loc[:, 'agnt_'].map(dict_agnt_prev_num_wc) 
